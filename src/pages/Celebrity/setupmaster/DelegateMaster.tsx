...
function DelegateMaster() {
  ...
  const getAll = async(magentoUserId)=>{
    console.log("userroles",celebUsersRoles);
    if (celebUsersRoles.length === 0) {
      await dispatch(getCelebUsersRolesList({}));  
      console.log("userroles1",celebUsersRoles);    
    }
    try{
...
const getJuniorsBySeniorRM = async(magentoUserId,userName)=>{
  ...
  const SAVERULE = () => {
    ...
    common.apiCall2('POST',`UserRegister/UpdateIsDelegate`, objDelegate)
      .then(res => {
        setisSettuped(false);
        let {data} = res;
    ...
    <Page className="TablePage">
      ...
      <Row>
        <Col>
          ...
          <CardHeader>Delegate Master</CardHeader>
